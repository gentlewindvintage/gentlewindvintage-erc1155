 ...
 */
contract ERC1155TradeableUpgradeable is
    ERC1155Upgradeable,
    OwnableUpgradeable,
    MinterRoleUpgradeable,
    WhitelistAdminRoleUpgradeable
{
    ...
    function setURI(uint256 _id, string memory _uri) public onlyOwner {
        require(_exists(_id), "ERC721Tradeable#uri: NONEXISTENT_TOKEN");
        _uriRecords[_id] = URIRecord({exists: true, uri: _uri});
    }
    ...
    function create(
        ...
        if (_initialSupply != 0) {
            require(
                ...
                "targets list must have same length as amounts list"
            );
